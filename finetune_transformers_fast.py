"""
CPU-OPTIMIZED Fine-tuning script with reduced dataset.
"""
...
def analyze_data(data_path, sample_fraction=1.0):
    ...
    if sample_fraction < 1.0:
        ...
        print(f"⚡ Reduced dataset: {original_size:,} → {len(df):,} samples ({sample_fraction*100:.0f}%)")
...
def plot_evaluation(y_true, y_pred, y_proba, metrics, save_path):
    ...
    axes[0, 1].plot(fpr, tpr, label=f'AUC = {metrics["roc_auc"]:.3f}', linewidth=2)
    ...
    axes[1, 0].plot(recall, precision, label=f'AUC = {metrics["pr_auc"]:.3f}', linewidth=2)
...
def fine_tune_model(train_loader, val_loader, test_loader, class_weights,
                    num_epochs=3, learning_rate=5e-5, results_dir=None):
    ...
    if results_dir:
        model_path = results_dir / 'distilbert_finetuned.pt'
        torch.save({
...
def main():
    """Main pipeline - CPU OPTIMIZED."""
    print("\n" + "=" * 70)
    print("CPU-OPTIMIZED TRANSFORMER FINE-TUNING")
    print("=" * 70)
    
    config = {
        'data_path': 'data/raw/cleaned_ai_human_dataset.csv',
        'sample_fraction': 0.2,    # ✅ Use 20% of data (10K samples)
    ...
    results_dir = Path('results') / 'finetuned_models_cpu' / timestamp
